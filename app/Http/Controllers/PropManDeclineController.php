...
class PropManDeclineController extends Controller
{
    ...
    public function deletesinglelandlord($id)
    {
        try {
            $landlordid = Crypt::decrypt($id);
            try {
                if (DB::table('propmanproperty')->select('id')->where(
                    'landlordid',
                    $landlordid
                )->where('available', '=', 'Y')->exists()) {
                    return  redirect()->route('propdec.listlanddec')
    ...
    public function disablelandlordcontact($id, $cid)
    {
        try {
            ...
            try {
                if (DB::table('propmanlease')->select('id')->where(
                    'landlordcontactid',
                    $contactid
                )->where('available', '=', 'Y')->exists()) {
    ...
    public function disablelandlord($id)
    {
        try {
            $landlordid = Crypt::decrypt($id);
            try {
                if (DB::table(table: 'propmanlandlordcontact')->select('id')->where(
                    'landlordid',
                    $landlordid
                )->where('available', '=', 'Y')->exists()) {
    ...
    public function vieweditsinglelease($id)
    {
        try {
            ...
            try {
                ...
                $arr['lease']   = DB::table('propmanalllease')->where(
                    'id',
                    $leaseid
                )->select(columns: '*')->first();
                $arr['prepay']   = DB::table('propmantempleaseprepayments')->where(
                    'leaseid',
                    $leaseid
                )->select('*')->get();
                $arr['balance']   = DB::table('propmantempleasearrearsdetails')->where(
                    'leaseid',
                    $leaseid
                )->select('*')->get();
                $arr['rates']   = DB::table('propmantempleasecurrentbillrates')->where(
                    'leaseid',
                    $leaseid
                )->select('*')->get();
                ...
                $arr['clienttype']   = DB::table('propmanalltenant')->where(
                    'id',
                    $arr['lease']->tenantid
                )->select('*')->first();
            ...
            try {
                ...
                DB::table('propmanlease')->where('id', $leaseid)
                    ->update([
                        ...
                        'areataken' => $request->areataken,
                        'rental' => $rental,
                        'currencycode' => $request->currencycode,
                        'ratesqm' => $request->expectedrate,
                        'propertydescription' => $request->propertydescription,
                        'rentreview' => $request->rentreviewperiod,
                        'inspectionreview' => $request->inspectionperiod,
                        'approval' => 'N',
    ...
    public function deletesingleproperty($id)
    {
        try {
            $propertyid = Crypt::decrypt($id);
            try {
                if (DB::table('propmanalllease')->select('id')->where(
                    'propertyid',
                    $propertyid
                )->where('available', '=', 'Y')->exists()) {
                    return  redirect()->route('propdec.listpropdec')
    ...
    public function updatesingletenant($id, Request $request)
    {
        try {
            ...
            try {
                ...
                DB::table('propmantenant')->where('id', $tenantid)->update([
                    'nationalid' => $request->nationalid,
                    'clienttypeid' => $request->clienttype,
                    'firstname' => $request->firstname,
                    'cell' => $request->cell,
            ...
            try {
                DB::table('propmaninvoicepre')->where('id', $invoiceid)
                    ->update([
                        ...
                        'rates' => $request->rateslevies,
                        'operationalcost' => $request->operationcosts,
                        'vat' => $request->vat,
                        'isedited' => 'Y',
                        'editedby' => session('alluser')
                    ]);
