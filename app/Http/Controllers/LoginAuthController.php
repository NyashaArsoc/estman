...
class LoginAuthController extends Controller
{
    ...
    public function userlogout()
    {
        if (session()->has('alluser')) {
            try {
                $logouttime = date("Y-m-d H:i:s", strtotime(
                    '+2 hours',
                    strtotime(now())
                ));
    ...
    public function profilepassword(Request $request)
    {
        try {
            $user = $this->userdetail();
            $passwordexist = $this->oldpasswordcheck($request->password, $user->username);
    ...
    public function changepasswordexpired($id, Request $request)
    {
        try {
            ...
            try {
                ...
                $license    =   $this->getlicensecheck();
                $login = collect(DB::select(
                    'EXEC  spPostUserLogin ?',
                    [$username]
                ))->first();
