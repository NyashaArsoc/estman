@php
$title = 'Edit Lease Rate';
$description = 'update lease details...';
$rid= Crypt::encrypt($rates->id);
...
<div class="container-fluid">
    ...
    <ol class="breadcrumb no-bg mb-1">
        ...
        <li class="breadcrumb-item"><a href="{{ route('propma.editlea',$id) }}">View/Edit</a></li>
        <li class="breadcrumb-item active">{{ $title }}</li>
    </ol>
    <div class="box box-block bg-white">
        <h5>{{ $title }}</h5>
        <p class="font-90 text-muted mb-1"> {{ $description }}</p>
        <span class="badge badge-pill bg-info">{{$lease->propertytype ?? '' }}</span>
        <hr />
        <form class="form-material material-primary" method="POST" id="defaultform"
            action="{{ route('propma.updtlearate',[ $rid,$id]) }}">@csrf
            ...
            <div class="form-group row">
                <label for="" class="col-sm-2 form-control-label">Rates/Utilities</label>
                <div class="col-sm-4">
                    <input type="text" class="form-control" id="leaseratescost"
                        name="leaseratescost" value="{{ $rates->ratescosts ?? ''}}">
                    <small id="leaseratescostcheck" style="color: red;"></small>
                </div>
                <label for="" class="col-sm-2 col-form-label">Operation Cost</label>
                <div class="col-sm-4">
                    <input type="text" class="form-control" id="leaseoperationalcost"
                        name="leaseoperationalcost" value="{{ $rates->operationalcosts ?? ''}}">
            ...
            <div class="form-group row">
                ...
                <div class="offset-sm-2 col-sm-10">
                    <button type="submit" class="btn btn-primary" id="btn-add-lease-rate">submit</button>
                </div>
                @endif
            </div>
