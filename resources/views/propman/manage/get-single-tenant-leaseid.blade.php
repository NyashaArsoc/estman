...
<table  class="table table-hover table-bordered">
    ...
    <tbody id="balancebd">
        @foreach($balances as $abc)<tr><td>{{ $abc->currencycode }}{{ $abc->precurrencycode }}</td>
            <td>{{ number_format($abc->balancebd,2) }}</td><td>{{ number_format($abc->prepayment,2) }}</td></tr>@endforeach
    </tbody>
</table>
<div class="b-a b-a-success b-a-width-1 mb-0-5"></div>
