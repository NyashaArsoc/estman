@php
    $title = 'Edited Profoma Invoice ';
    $description = 'updated invoice to be generated...';
...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <ol class="breadcrumb no-bg mb-1">
            ...
            <li class="breadcrumb-item"><a href="{{route('invoice.listeditedprofoma')}}">Pending</a></li>
            <li class="breadcrumb-item active">{{ $title }}</li>
        </ol>
        <div class="box box-block bg-white">
            <h5>{{ $invoice->currencycode.' - '.$title }}</h5>
            <p class="font-90 text-muted mb-1"> {{ $description }}</p>
            <form class="form-material material-primary" action="{{ route('invoice.approveeditedprofoma', $id) }}"
            method="PUT"> @csrf
